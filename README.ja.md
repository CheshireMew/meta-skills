...
Meta-skills は、Codex/Agent の Skill を作成・改善するための Skill です。アイデア、資料、または既存の Skill を渡すと、新しい Skill を一式作ったり、既存の Skill がうまく動かない理由を調べたり、別の場所で実証済みの方法を移したりできます。
...
| --- | --- |
| アイデアはあるが Skill はまだない | 必要なファイルと呼び出し方法を備えた Skill を作る |
...
記事を一本書く、データを分析する、関数を一つ直すなど、一度限りの作業を完了したいだけで Skill を作成・変更しない場合、Meta-skills は不要です。その分野を担当する Skill を使うか、Codex に直接依頼してください。
