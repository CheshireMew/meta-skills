...
def validate_meta_method_selection_contract(skill_text: str) -> list[str]:
    ...
    required_markers = (
        ...
        "高风险或外部动作",
        "### 2. 防止下层架空上层决定",
        "用户没有点名改变或退出的部分作为本轮不变量保留",
        "下层是否扩大了方法、输入、输出、权限或验证强度",
        "反向检查上层重写有没有遗漏、缩小或退出下层原本提供的有效能力",
        "恢复它在上层的正式入口",
        ...
        "上层已经正确时，直接修下层路由或消费者",
        "混合任务逐阶段验证",
        "目标 Skill 已有 Git 仓库和配置好的 GitHub upstream",
        "用户选择只改本地时才停止在本地",
        "已有 GitHub upstream 且未选择只改本地时能够到达精确提交、推送和远端 HEAD",
        "默认只暂存本轮获准依赖闭包",
        "创建远端、改变可见性、强制推送、删除和其它未获准外部动作不包含在这个默认阶段",
    )
