...
Meta-skills 是一个专门用来创建和改进 Codex/Agent Skill 的 Skill。你可以把想法、资料或一个已经存在的 Skill 交给它，让它帮你做出完整的新 Skill，找出旧 Skill 为什么不好用，或者把一套有效做法迁移进去。
...
| --- | --- |
| 只有一个想法，还没有 Skill | 创建可安装的 Skill，并补齐使用入口和必要文件 |
...
Meta-skills 会先读懂现状，再把方案讲清楚。你确认之前，它不会改活动文件；你确认之后，它会完成修改和对应检查。目标 Skill 已经在远端仓库中时，它默认继续提交并推送本轮确认的改动；如果你只想改本地，在确认方案时说明即可。
...
如果你只是想完成一次普通任务，例如写一篇文章、分析一份数据或修一个函数，而不准备创建或改变 Skill，就不需要 Meta-skills。直接使用负责那类任务的 Skill 或让 Codex完成即可。
